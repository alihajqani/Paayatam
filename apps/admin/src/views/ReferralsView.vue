...
async function decide(note: string): Promise<void> {
  ...
  try {
    if (pending.value.kind === 'reject') {
      ...
      notice.value =
        'معرفی به حالت «در انتظار» بازگشت. این کار پاداشی پرداخت نمی‌کند — شرط حضور در یک رویداد همچنان باید محقق شود.';
    }
    pending.value = null;
    await load();
  } catch (cause) {
...
<template>
  ...
  <ConfirmDialog
    ...
    :body="
      pending?.kind === 'reject'
        ? 'پاداشی برای این معرفی ثبت نمی‌شود، حتی اگر کاربر بعداً در رویدادی حاضر شود. این تصمیم برگشت‌پذیر است.'
        : 'معرفی دوباره «در انتظار» می‌شود. این کار به‌تنهایی پاداشی پرداخت نمی‌کند؛ شرط حضور در یک رویداد همچنان بررسی می‌شود.'
    "
