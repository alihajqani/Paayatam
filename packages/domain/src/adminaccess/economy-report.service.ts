...
export class EconomyReportService {
  ...
  private conversion(row: { converted: number; eligible: number }): EconomyMetric {
    ...
    return {
      ...
      target: 'بیشتر از ۳۵٪',
      meaning:
        'از میان کسانی که در دست‌کم سه رویداد شرکت کرده‌اند، چند درصد سکه خریده‌اند. ' +
        'می‌گوید محصول ارزشش را ثابت کرده یا نه.',
      advice:
        'اگر پایین است، مسئله قیمت نیست بلکه ارزش است: کسی که سه شب بیرون رفته و ' +
        'حاضر نیست پول بدهد، دارد دربارهٔ کیفیت رویدادها حرف می‌زند نه دربارهٔ قیمت.',
      sample: row.eligible,
    };
  }
  ...
  private fill(row: { accepted: number; capacity: number; events: number }): EconomyMetric {
    ...
    return {
      key: 'fill_rate',
      label: 'نرخ پرشدن رویداد',
      value,
      kind: 'ratio',
      status: value === null ? 'unknown' : value > 0.6 ? 'good' : value > 0.4 ? 'warn' : 'bad',
      target: 'بیشتر از ۶۰٪',
      meaning:
        'صندلی‌های پرشده تقسیم بر صندلی‌های عرضه‌شده، در رویدادهایی که برگزار شده‌اند. ' +
        'رویدادهای بدون محدودیت ظرفیت شمرده نمی‌شوند، چون «پر شدن» برایشان معنا ندارد.',
      advice:
        ...
        'موقتاً پایین بیاورید و همهٔ انرژی را روی ساختن رویداد بگذارید.',
      sample: row.events,
    };
  }
